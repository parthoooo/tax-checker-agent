...
const AnalysisSummary: React.FC<Props> = ({ result, loading }) => {
  ...
  const normalized = normalizeComparisonResult(result as ComparisonResult & Record<string, unknown>);

  const hasIssues =
    normalized.missing.length > 0 ||
    normalized.wrongYear.length > 0 ||
    normalized.wrongType.length > 0 ||
    normalized.unexpected.length > 0 ||
    normalized.yoyNotes.length > 0;
  ...
  return (
    <Card className={`mb-8 ${hasIssues ? 'border-amber-300' : 'border-green-300'}`}>
      ...
      <CardContent className="space-y-4">
        ...
        {!hasIssues && normalized.verified.length > 0 && (
