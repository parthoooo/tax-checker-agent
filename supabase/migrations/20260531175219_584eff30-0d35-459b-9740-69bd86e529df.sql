...
-- email_drafts (may already exist from 002_prototype_additions)
...
ALTER TABLE public.email_drafts ADD COLUMN IF NOT EXISTS created_by uuid;
...
DROP POLICY IF EXISTS "admin all email_drafts" ON public.email_drafts;
...
CREATE INDEX IF NOT EXISTS idx_email_drafts_client ON public.email_drafts(client_id);

-- time_entries (may already exist from 002_prototype_additions)
...
DROP POLICY IF EXISTS "admin all time_entries" ON public.time_entries;
